...
#define XEMBED_EMBEDDED_VERSION (VERSION_MAJOR << 16)
  ...
  if (display != NULL) {
    ...
    if (resm != NULL) {
      ...
      if (xrdb != NULL) {
        ...
        XRDB_LOAD_COLOR("color3", selbgcolor);
...
tile(Monitor *m)
{
  ...
  for (i = 0, my = ty = m->gappx, c = nexttiled(m->clients); c;
    ...
    if (i < m->nmaster) {
      ...
      for (j = i, tmp = c; tmp && j < m->nmaster; tmp = nexttiled(tmp->next), j++)
